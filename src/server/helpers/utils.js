...
const scriptImport = (src) =>
  src ? `<script type="text/javascript" src="${src}"></script>` : "";

const faviconImport = (href) =>
  href ? `<link rel="icon" href=${href} type="image/x-icon" />` : "";
...
const setGlobalGO = (opt, experiments) =>
  experiments
    ? `<script src="https://www.googleoptimize.com/optimize.js?id=${opt}"></script>`
    : "";

const configGlobalGOExperiments = (experiments) =>
  experiments && experiments.length > 0
    ? `<script>
  ${experiments.map(
    (e) =>
      `gtag('set', {'expId': '${e.id}'}); gtag('set',  {'expVar': '${e.value}'});`
  )}
  </script>`
    : "";
